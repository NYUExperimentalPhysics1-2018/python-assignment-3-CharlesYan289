...
def trajectory (x0,y0,v,theta,g = 9.8, npts = 1000):
    ...
    #yfinal=y0+vy0*tfinal-1/2*g*(tfinal**2)
    #print(yfinal)
...
def tankShot (targetBox, obstacleBox, x0, y0, v, theta, g = 9.8):
    ...
    hitob=firstInBox(x,y,obstacleBox)
    #print(hitob)
    if hitob>=0:
...
def oneTurn (tank1box, tank2box, obstacleBox, playerNum, g = 9.8):   
    ...
    #print("yes")
    angle=getNumberInput("player"+str(playerNum)+" what is your angle",[0,360])
    velocity=getNumberInput("player"+str(playerNum)+" what is your velocity",[0,np.inf])
    if(playerNum==1):
        origin=tank1box
        target=tank2box
    else:
...
def playGame(tank1box, tank2box, obstacleBox, g = 9.8):
    ...
    while True:
        ...
        if(result>0):
            break
        passturn=input("press enter to continue")
        playerNum= (playerNum%2) +1
        
    print("congratulation Player ",str(playerNum),", you win!")
